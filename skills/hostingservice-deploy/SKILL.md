...
从 `https://github.com/inkfin/HostingService.git` 克隆部署仓库。仓库已按用户要求公开，下载无需认证。白板受支持 VPS 默认运行 README 的 install.sh，安装 Homebrew/OpenCode 并引导模型登录；Codex 由用户以后自行安装。安装器在用户选择和模型认证步骤等待输入，完成后交接 agent。已有电脑 agent 经 SSH 接管时可直接补齐目标机依赖，无需额外安装服务器 agent。OpenCode 模型凭据留在普通用户 home，系统操作按需 sudo，不运行 root Homebrew。已有 checkout 先检查改动和版本，不覆盖 `.env`、`runtime/`、`data/`，不运行清库命令。安装后的 skill 目录可能独立于源码，不能用 `../../` 猜测部署代码位置。
